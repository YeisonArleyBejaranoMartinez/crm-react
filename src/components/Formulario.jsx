...
const Formulario = ({ cliente, cargando }) => {
  ...
  const nuevoClienteSchema = Yup.object().shape({
    nombre: Yup.string().required("El nombre del cliente es obligatorio.").min(3, "El nombre es muy corto").max(20, "El nombre es muy largo"),
  ...
  return (
    cargando ? <Spinner /> : (
      <div className='bg-white mt-10 px-5 py-10 rounded-md shadow-md'>
        <h1 className='text-gray-600 font-bold text-xl uppercase text-center md:w-3/4 mx-auto'>{cliente?.nombre ? "Editar cliente" : "Agregar Cliente"}</h1>
        <Formik
          initialValues={{
